...
def do_code_lookup(message: str) -> Dict[str, Any]:
    ...
    if embedder is None:
        raise RuntimeError("Embedding backend not available. Set EMBEDDING_BACKEND and required env variables.")
...
def _llm_summarize(
    ...
    # Log configuration before making the call
    logger.info(f"[LLM_CONFIG] Model: {LLM_MODEL}, Timeout: {LLM_TIMEOUT_MS}ms ({LLM_TIMEOUT_MS / 1000.0}s)")
    ...
    try:
        ...
        logger.error(f"[LLM_ERROR] LLM call failed with error: {e}", exc_info=True)
        raise

...
def perform_search(
    ...
    if unified_router_available and os.getenv("VECTOR_BACKEND", "faiss") == "milvus":
        ...
        if not should_use_faiss or fallback:
            ...
            # Encode query and normalize to 1D float32
            ...
            try:
                ...
                try:
                    ...
                    if thread.is_alive():
                        ...
                        # Reorder results based on reranked texts (simplified: keep original order for now)
                        # TODO: Map reranked_texts back to results by matching text
                        reranker_info = {
                            "enabled": True,
                            "triggered": True,
                            "margin": round(margin, 4),
                        ...
                        logger.info(f"[RERANK] Completed in {rerank_latency_ms:.1f}ms")
                    else:
                        # Unexpected case
                        rerank_timeout = True
                        reranker_info = {

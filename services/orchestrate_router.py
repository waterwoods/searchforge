...
from agents.orchestrator.flow import ExperimentPlan, OrchestratorFlow, DatasetBlockError, AlignmentBlockError
...
def orchestrate_run(
    ...
    try:
        ...
        if payload.get("preset") or payload.get("collection"):
            ...
            # Support both old format (list) and new format (dict with default/allowed)
            collections_cfg = config.get("collections", {})
            if isinstance(collections_cfg, dict):
                collection = payload.get("collection") or collections_cfg.get("default", "fiqa_para_50k")
            else:
                # Legacy format: list
                collection = payload.get("collection") or (collections_cfg or ["fiqa_para_50k"])[0]
    ...
    try:
        ...
        return result
    except DatasetBlockError as exc:
        raise HTTPException(
            status_code=409,
            detail={
                "code": exc.code,
    ...
    except AlignmentBlockError as exc:
        raise HTTPException(
            status_code=409,
            detail={
                "code": exc.code,
...
def orchestrate_status(
    run_id: str = Query(..., description="Unique orchestrator run_id"),
    detail: str = Query("lite", description="Detail level: 'lite' or 'full'"),
) -> Dict[str, Any]:
    try:
        if detail not in ("lite", "full"):
            detail = "lite"  # Default to lite if invalid
        return _orchestrator_flow.get_status(run_id, detail=detail)

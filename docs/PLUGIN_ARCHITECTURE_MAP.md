...
| Topic | Detail |
|-------|--------|
| **Current** | `active_vehicle_resolver.resolve_add_car_active_vehicle` (deterministic segments + ambiguity), **unit tests only** — `triage.py` does not import it yet. |
| **Gap** | Either **wire** as the single add-car segment resolver **or** delete and fold tests into triage — see `docs/DEPRECATED_PATHS.md`. |
| **Test** | Unit tests in `tests/test_active_vehicle_resolver.py` + live chaos multi-vehicle sessions. |

# Legacy README — SearchForge Lab / Code Lookup Agent

> **HISTORICAL / LAB ONLY.** This is the pre–Unified Intake README (SearchForge R&D, GPU worker, Metrics Hub, AutoTuner, Code Lookup Agent).  
> **Do not use for paid pilot or broker ops.**  
> **Operator truth:** [`docs/CURRENT_PRODUCT_SHAPE.md`](../CURRENT_PRODUCT_SHAPE.md) · [`docs/runbooks/OPERATOR_CHEAT_SHEET.md`](../runbooks/OPERATOR_CHEAT_SHEET.md)
...
## Unified Intake — California Auto Insurance (Product)

**What we ship:** broker intake SaaS — paste → classify → case → workbench. Postgres-backed cases; vectors optional.

| Task | Command |
|------|---------|
| Local workbench | `bash scripts/run_demo_local.sh` → http://localhost:5173/workbench/unified-intake |
| Paid pilot deploy | `bash scripts/deploy_paid_pilot.sh` (after `validate_pilot_deploy_env.py`) |
| Runtime truth | [docs/CURRENT_PRODUCT_SHAPE.md](docs/CURRENT_PRODUCT_SHAPE.md) |
| 2am ops | [docs/runbooks/OPERATOR_CHEAT_SHEET.md](docs/runbooks/OPERATOR_CHEAT_SHEET.md) |
| What to ignore | [docs/runbooks/OPERATOR_IGNORE_LIST.md](docs/runbooks/OPERATOR_IGNORE_LIST.md) |

**Agents (Cursor/OpenClaw):** [AGENTS.md](AGENTS.md) — default entry point.  
**Founder quick start:** [docs/ANDY_QUICK_START.md](docs/ANDY_QUICK_START.md)

Optional RAG demo page: http://localhost:5173/demo (lab wedge — not the intake product).

---

## Quick Start — Legacy Docker / SearchForge lab (optional)

> **Not the paid pilot path.** Use `run_demo_local.sh` + `deploy_paid_pilot.sh` for Unified Intake.

...
def test_mvp_sendable_includes_vin_vehicle_information_and_insurance_card():
    assert is_mvp_sendable_item_type("vin") is True
    assert is_mvp_sendable_item_type("VIN") is True
    assert is_mvp_sendable_item_type("vehicle_information") is True
...
def test_checklist_flags_mvp_sendable_vin_and_insurance_card():
    ...
    assert by_key["vehicle_information"]["mvp_sendable"] is True
    assert by_key["vehicle_information"]["item_type"] == "vehicle_information"
    assert by_key["vehicle_information"]["business_class"] == "request_more"
    assert by_key["vehicle_information"]["suggested_for_request"] is True
...
def test_list_unsupported_send_item_labels():
    labels = list_unsupported_send_item_labels(
        [
            ...
            {"item_type": "vehicle_information", "label": "车辆信息"},
            {"item_type": "free_text", "label": "Other free text"},
        ]
    )
    assert labels == ["Other free text"]

...
export const CLAIM_REQUEST_MORE_COPY = {
  ...
  retry: '重试',
  saveDraftNow: '立即保存草稿',
  aiDraftButton: 'AI帮我起草',
  aiDraftRegenerate: '重新起草',
  aiDraftUseTemplate: '改用标准模板',
  aiDrafting: 'AI起草中…',
  aiDraftMissingCount: (count: number) => `还缺 ${count} 项`,
  aiDraftApplied: '草稿已填入，请核对或修改后再发出。',
  aiDraftFallbackApplied: 'AI暂时不可用，已填入标准模板，请核对后再发出。',
  aiDraftNothingMissing: '无需补充 — 当前没有需要客户补充的项目。',
  aiDraftBlockedOpenRequest: '已有未完成的补充请求，请先等待客户完成或核对后再起草。',
  aiDraftFailed: '暂时无法起草。请手动填写或稍后重试。',
  aiDraftPreviewTitle: '给客户的补充说明（预览）',
  aiDraftReviewHint:
    '预览仅供核对。客户实际看到的是下方每一项的标题和说明，可直接修改；发出仍由您点击「发出补充请求」。',

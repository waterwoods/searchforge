...
// ----------------------------------------------------
const BASE = import.meta.env.VITE_API_BASE || '/orchestrate';
const API_URL = BASE.startsWith('/') ? BASE : `/${BASE}`;
...
const StewardDashboard = () => {
    ...
    const handleFetchReport = async (id) => {
        ...
        try {
            // Switch to real API
            // const data = await fetch(`${API_URL}/report?run_id=${id}`).then(res => res.json());
    ...
    const handlePollStatus = (id) => {
        pollIntervalRef.current = setInterval(async () => {
            try {
                ...
                const data = await fetch(`${API_URL}/status?run_id=${id}&detail=${detailLevel}`).then(res => res.json());
    ...
    const handleRun = async (preset = 'smoke', overrides = {}) => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/run?commit=true`, {
                method: 'POST',
    ...
    const handleAbort = async () => {
        ...
        stopPolling();
        // Switch to real API
        // await fetch(`${API_URL}/abort?run_id=${runId}`, { method: 'POST' });

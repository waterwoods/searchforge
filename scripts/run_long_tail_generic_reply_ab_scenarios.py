#!/usr/bin/env python3
"""
Long-tail generic reply A/B scenarios.

Runs docs/sprints/LONG_TAIL_GENERIC_REPLY_EXTERNALIZATION_SPRINT/long_tail_ab_scenario_battery.json
with explicit client_id (no reliance on CLIENT_ID env).

Usage:
  LLM_GENERATION_ENABLED=0 PYTHONPATH=. python3 scripts/run_long_tail_generic_reply_ab_scenarios.py [--verbose]
"""
...
BATTERY = (
    ...
    / "docs"
    / "sprints"
    / "LONG_TAIL_GENERIC_REPLY_EXTERNALIZATION_SPRINT"
...
def run_one(s: dict, verbose: bool) -> tuple[bool, str | None]:
    from services.fiqa_api.inbox_triage.triage import _rule_based_triage, triage_conversation
    ...
    use_rule_based = bool(s.get("use_rule_based"))
    if use_rule_based:
        merged = latest
        if turns:
            merged = "\n".join([t["text"] for t in turns if t.get("text")] + [latest]).strip()
        result = _rule_based_triage(merged, client_id=client_id or None)
    else:
...
def main() -> int:
    ...
    if failed:
        print(f"Long-tail generic reply A/B: {failed}/{len(scenarios)} failed", file=sys.stderr)
        return 1

    print(f"Long-tail generic reply A/B: {len(scenarios)} scenarios OK")
    return 0

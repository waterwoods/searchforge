...
Usage:
  LLM_GENERATION_ENABLED=0 PYTHONPATH=. python3 scripts/run_residual_copy_ab_scenarios.py [--verbose]
"""
...
def main() -> int:
    ...
    if failed:
        print(f"Residual copy A/B: {failed}/{len(scenarios)} failed", file=sys.stderr)
        return 1

    print(f"Residual copy A/B: {len(scenarios)} scenarios OK")
    return 0

#!/usr/bin/env python3
"""
Simple Macro Knobs Test Script

This script runs a quick test for the macro knobs feature without reranking
to avoid Hugging Face connectivity issues.
"""
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('macro_knobs_simple_test.log'),
            logging.StreamHandler()
        ]
    )
    return logging.getLogger(__name__)

def run_test_case(case_name: str, latency_guard: float, recall_bias: float, duration_sec: int = 60):
    ...
    env.update({
        ...
        "FORCE_HYBRID_ON": "0",  # Disable hybrid to avoid BM25 issues
        "FORCE_CE_ON": "0",      # Disable reranking to avoid HF issues
        "CE_CACHE_SIZE": "0",
        "RERANK_K": "50",
        "CANDIDATE_K_STEP": "200",
        "CHAOS_LAT_MS": "0",     # Disable chaos for cleaner results
    ...
    # Create a simple config for the search pipeline (no reranker)
    config = {
        "retriever": {
            "type": "vector",
            "top_k": 200
        }
        # No reranker to avoid HF connectivity issues
    }
    ...
    while time.time() - start_time < duration_sec:
        for query in test_queries:
            ...
            try:
                ...
                query_count += 1
                logger.info(f"Query {query_count}: {search_time:.1f}ms, {len(search_results)} results")
...
<body>
    ...
    <script>
        ...
        const allTimestamps = {json.dumps([r['timestamp'] for r in results_a + results_b])};
        const startTime = allTimestamps.length > 0 ? Math.min(...allTimestamps) : 0;
        
        // Sample data points to reduce density - take every 5th point
        const dataA = {json.dumps([{'x': round((r['timestamp'] - min([r['timestamp'] for r in results_a + results_b])), 1), 'y': r['search_time_ms']} for i, r in enumerate(results_a) if i % 5 == 0])};
        const dataB = {json.dumps([{'x': round((r['timestamp'] - min([r['timestamp'] for r in results_a + results_b])), 1), 'y': r['search_time_ms']} for i, r in enumerate(results_b) if i % 5 == 0])};
...
def main():
    ...
    logger.info("Simple Macro Knobs Test completed successfully!")

...
if [ -n "${ACCIDENT_STORY_LANGSMITH_PROJECT:-}" ]; then
    ENV_VARS+=("ACCIDENT_STORY_LANGSMITH_PROJECT=$ACCIDENT_STORY_LANGSMITH_PROJECT")
fi
...
# AI Request More drafting — kill switches (Cloud QA). Unset = office template only.
if [ -n "${REQUEST_MORE_ASSISTANT_ENABLED:-}" ]; then
    ENV_VARS+=("REQUEST_MORE_ASSISTANT_ENABLED=$REQUEST_MORE_ASSISTANT_ENABLED")
fi
if [ -n "${REQUEST_MORE_ASSISTANT_LLM:-}" ]; then
    ENV_VARS+=("REQUEST_MORE_ASSISTANT_LLM=$REQUEST_MORE_ASSISTANT_LLM")
fi
if [ -n "${REQUEST_MORE_ASSISTANT_OFFICE_ALLOWLIST:-}" ]; then
    ENV_VARS+=("REQUEST_MORE_ASSISTANT_OFFICE_ALLOWLIST=$REQUEST_MORE_ASSISTANT_OFFICE_ALLOWLIST")
fi
if [ -n "${REQUEST_MORE_ASSISTANT_MODEL:-}" ]; then
    ENV_VARS+=("REQUEST_MORE_ASSISTANT_MODEL=$REQUEST_MORE_ASSISTANT_MODEL")
fi
if [ -n "${REQUEST_MORE_ASSISTANT_TIMEOUT_SECONDS:-}" ]; then
    ENV_VARS+=("REQUEST_MORE_ASSISTANT_TIMEOUT_SECONDS=$REQUEST_MORE_ASSISTANT_TIMEOUT_SECONDS")
fi
if [ -n "${REQUEST_MORE_ASSISTANT_MAX_RETRIES:-}" ]; then
    ENV_VARS+=("REQUEST_MORE_ASSISTANT_MAX_RETRIES=$REQUEST_MORE_ASSISTANT_MAX_RETRIES")
fi

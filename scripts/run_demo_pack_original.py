...
class DemoPackOrchestrator:
    ...
    def _run_simulation_experiments(self, scenario: str, duration_sec: int, bucket_sec: int, 
        ...
        single_knob_dir = scenario_dir / "single_knob"
        single_knob_dir.mkdir(exist_ok=True)
        
        print(f"   🔴 Running Single-Knob simulation...")
        single_knob_cmd = [
            sys.executable, "scripts/run_brain_ab_experiment.py",
            "--mode", "simulator",
            "--duration-sec", str(duration_sec),
            "--bucket-sec", str(bucket_sec),
            "--csv-out", str(single_knob_dir / "data.csv"),
            "--seed", str(seed)
        ]
        
        result = subprocess.run(single_knob_cmd, capture_output=True, text=True)
        if result.returncode != 0:
            raise RuntimeError(f"Single-knob simulation failed: {result.stderr}")
        
        # Find the generated directories
        reports_dir = Path("reports/observed/ab_effectiveness")
        single_knob_exp_dir = None
        multi_knob_exp_dir = None
        
        for exp_dir in reports_dir.glob("*"):
            if "single_knob" in exp_dir.name:
                single_knob_exp_dir = exp_dir
            elif "multi_knob" in exp_dir.name:
                multi_knob_exp_dir = exp_dir
        
        if not single_knob_exp_dir or not multi_knob_exp_dir:
            raise RuntimeError("Could not find generated experiment directories")
        
        # Copy results to scenario directory
        shutil.copytree(single_knob_exp_dir, single_knob_dir / "experiment", dirs_exist_ok=True)
        shutil.copytree(multi_knob_exp_dir, scenario_dir / "multi_knob" / "experiment", dirs_exist_ok=True)
        
        # Load metrics
        with open(single_knob_exp_dir / "metrics.json") as f:
            single_knob_metrics = json.load(f)
        
        with open(multi_knob_exp_dir / "metrics.json") as f:
            multi_knob_metrics = json.load(f)
        
        return {
            "single_knob": {
                "metrics": single_knob_metrics,
                "experiment_dir": str(single_knob_dir / "experiment")
            },
            "multi_knob": {
                "metrics": multi_knob_metrics,
                "experiment_dir": str(scenario_dir / "multi_knob" / "experiment")
            },
            "comparison": {
                "delta_p95_ms": single_knob_metrics.get("delta_p95_ms", 0),
                "delta_recall": single_knob_metrics.get("delta_recall", 0),
                "p_value": single_knob_metrics.get("p_value", 1.0),
                "seed": seed
            }
        }
    ...
    def _generate_scenario_report(self, scenario: str, results: Dict[str, Any]):
        ...
        # Generate one_pager report using existing aggregate_observed.py
        if results["mode"] == "sim" and "experiment_dir" in results["single_knob"]:
            
            single_dir = results["single_knob"]["experiment_dir"]
            multi_dir = results["multi_knob"]["experiment_dir"]
            
            # Call aggregate_observed.py to generate report
            cmd = [
                sys.executable, "scripts/aggregate_observed.py",
                "--simulator-ab", single_dir, multi_dir,
                "--output", str(scenario_dir / "one_pager.html")
            ]
            
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode != 0:
                print(f"Warning: Failed to generate scenario {scenario} report: {result.stderr}")
    ...
    def _create_index_html(self, global_metrics: Dict[str, Any], compare_mode: str) -> str:
        ...
        # Enhanced HTML template with proper styling and assets
        html = f"""
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>AutoTuner Demo Pack - {datetime.now().strftime('%Y-%m-%d %H:%M')}</title>
...
<body>
    <div class="header">
        ...
        <p><strong>Git SHA:</strong> {self.metadata['git_sha']}</p>
        <p><strong>Notes:</strong> {self.notes or 'No notes provided'}</p>
        <p><strong>Scenarios Run:</strong> {', '.join(self.metadata['scenarios_run'])}</p>
    </div>
    
    <div class="scenario-tabs">
        {"".join([f'<button class="tab" onclick="showScenario(\'{scenario}\')">Scenario {scenario}</button>' for scenario in self.results.keys()])}
        <button class="tab" onclick="showGlobal()">Global Comparison</button>
    </div>
    
    {"".join([self._create_scenario_html(scenario, results) for scenario, results in self.results.items()])}
    ...
    <div id="global" class="scenario-content">
        ...
        <div class="global-comparison">
            ...
            <div class="metric-card">
                <p><strong>Average ΔP95:</strong> {global_metrics['global_comparison']['single_vs_multi']['avg_delta_p95']:.2f} ms</p>
                <p><strong>Average ΔRecall:</strong> {global_metrics['global_comparison']['single_vs_multi']['avg_delta_recall']:.3f}</p>
                <p><strong>Average Apply Rate:</strong> {global_metrics['global_comparison']['single_vs_multi']['avg_apply_rate']:.2f}</p>
                <p><strong>Average Safety Rate:</strong> {global_metrics['global_comparison']['single_vs_multi']['avg_safety']:.2f}</p>
            </div>
        </div>
    </div>

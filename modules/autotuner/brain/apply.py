...
# Per-tick max step caps (anti-jitter)
STEP_CAPS = {
    "ef_search": 16,
    "ef": 16,
    "candidate_k": 200,
...
# Consecutive cap warning threshold
CONSECUTIVE_CAP_WARN_THRESHOLD = 3

def cap_step(prev: int, new: int, limit: int) -> int:
    """Cap per-tick delta to prevent jitter"""
    delta = new - prev
    if abs(delta) > limit:
        return prev + (limit if delta > 0 else -limit)
    return new
...
# Track consecutive cap counts per parameter (for warning)
_cap_history = {}
...
def reset_apply_counters():
    ...
    global _apply_counters, _cap_history
    _apply_counters = {
        "clipped_count": 0,
        "rejected_by_joint": 0,
        "rollback_count": 0,
...
def apply_updates(current_params: Dict[str, Any], updates: Dict[str, Any], mode: str, 
    ...
    if mode == "sequential":
        ...
        # Apply per-tick step caps (anti-jitter)
        capped = False
        for key in new_params:
            if key in STEP_CAPS:
                prev_val = current_params.get(key, new_params[key])
                requested_val = new_params[key]
                capped_val = cap_step(prev_val, requested_val, STEP_CAPS[key])
                
                req_delta = requested_val - prev_val
                app_delta = capped_val - prev_val
                was_capped = (capped_val != requested_val)
                
                # LOG_SPAWN_POINT: record every parameter update
                if key in feasible_updates:
                    print(f"[LOG_SPAWN_POINT] {key} req:{req_delta:+.0f} → app:{app_delta:+.0f} (capped={was_capped}) ts={time.time():.2f}")
                
                # Track consecutive caps and warn if threshold exceeded
                if was_capped:
                    capped = True
                    _cap_history[key] = _cap_history.get(key, 0) + 1
                    if _cap_history[key] >= CONSECUTIVE_CAP_WARN_THRESHOLD:
                        print(f"[WARN] {key} 连续{_cap_history[key]}次被封顶，可能目标太激进或步长上限过小。")
                else:
                    _cap_history[key] = 0  # Reset on success
                
                new_params[key] = capped_val
        ...
        # Include capped flag in result
        result = MultiKnobResult(
            status="applied",
            params_before=params_before,
            params_after=new_params,
            updates_applied=feasible_updates.copy()
        )
        # Store capped flag in params_snapshot metadata
        if not hasattr(result, 'metadata'):
            result.metadata = {}
        result.metadata['capped'] = capped
        return result

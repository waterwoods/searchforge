...
class SearchPipeline:
    ...
    def search(self, query: str, collection_name: str = "documents", candidate_k=None, trace_id=None, **kwargs) -> List[ScoredDocument]:
        ...
        if self.reranker and docs:
            ...
            # TRACE_E2E: Log RERANK args before calling reranker
            print(f"TRACE_E2E: RERANK args before calling reranker: {{query: '{query}', docs_count: {len(docs)}, rerank_k: {rerank_k}, rerank_kwargs: {rerank_kwargs}}}")
...
def search_with_explain(
    ...
    **kwargs
    ...
    """
    # Initialize search pipeline with configuration
    config = {
        "retriever": {
            "top_k": candidate_k if candidate_k is not None else 50,
            "collection_name": collection_name
        },
        "reranker": {
            "name": reranker_name
        },
        "rerank_k": rerank_top_k if rerank_top_k is not None else 50
    }
    ...
    if autotuner_params:
        ...
        if nprobe is not None:
            config["retriever"]["nprobe"] = nprobe
        if ef_search is not None:
            config["retriever"]["ef_search"] = ef_search
